...
`notebooks/rm1/tiktok_coordination_analysis.ipynb`), serta **goals** (dipetakan ulang pada Sentimen final
melalui artefak beku di `output/rm2_sentiment/final/` dan notebook pelaporan
`notebooks/rm2/02_rm2_sentiment_analysis.ipynb`; notebook V1 berada di
...
`output/rm2_sentiment/validation/human_v2/sentiment_human_annotation_v2_validated.csv`, ditambah delapan
...
`output/rm2_sentiment/validation/human_v2/locked_test_v2_observational_final.csv` dengan `300` komentar
...
Full inference V2 sudah dijalankan setelah acceptance gate lulus. Output utama disimpan di
`output/rm2_sentiment/final/`. Denominator utama adalah `33063` komentar observasional. Distribusi
observasional V2: Positive `2718` (`8.22%`), Neutral `23977` (`72.52%`),
...
| `FINAL_SENTIMENT_ANALYSIS_SUMMARY.csv` | Ringkasan | Status final, threshold, locked-test metrics, dan distribusi utama |
...
| `tables/account_sentiment_summary_v2.csv` | Akun | Ringkasan sentimen final pada level akun |
...
### Output Visualisasi Final (`output/rm2_sentiment/final/visualisasi/`, PNG saja)
...
dipertahankan. WordCloud eksploratif untuk paparan temuan tersedia terpisah di
`output/rm2_sentiment/legacy/exploratory/visualisasi/` dan tidak digunakan sebagai evidence validasi model.

### Output Gephi RM2

Output final V2 berada di `output/rm2_sentiment/final/gephi/`: `gephi_hcc_nodes_sentiment_v2.csv` (node HCC RM1 + atribut
...
Gunakan pemeriksa struktur notebook dan rencana output berikut untuk audit ringan:
...
melakukan full inference. Catatan audit historis repository berada di `docs/repository_audit/`.
...
`output/rm2_actor_type/gephi/`, atau `output/rm2_sentiment/final/gephi/`; gunakan project file hanya
