...
COMMENT_SENTIMENT_PATH = ROOT / "output/rm2_sentiment/legacy/v1/tables/comment_sentiment.csv"
...
OUT_DIR = ROOT / "output/rm2_sentiment/validation/human_v2"
...
def build_integrity_report(dataset: pd.DataFrame, comments: pd.DataFrame, hcc_nodes: pd.DataFrame, actor: pd.DataFrame) -> pd.DataFrame:
    ...
    checks = [
        ...
        (
            "safe_clean preserves validation/human_v2",
            "validation/human_v2 protected",
            "validation/human_v2" if "validation/human_v2" in pipeline_text else "missing",
            "validation/human_v2" in pipeline_text,
            "",
        ),
    ]
    report = pd.DataFrame(
...
Current V1 exclusion set: {v1_count} unique comment_id values from `output/rm2_sentiment/validation/human_v1/sentiment_human_annotation_validated.csv`. The V1 file is preserved and not overwritten by this package.
...
Daftar eksklusi V1 saat ini berisi {v1_count} nilai `comment_id` unik dari `output/rm2_sentiment/validation/human_v1/sentiment_human_annotation_validated.csv`. File V1 dipertahankan dan tidak ditimpa oleh paket ini.
