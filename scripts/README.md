...
| `open_tiktok_similarity_comments.py` | RM2 comment similarity review | LOCAL_BROWSER_REVIEW_AID | `comment_similarity_screenshot_queue.csv` | local lookup status/screenshots | Yes | No | No | No | Opens direct TikTok candidate URLs in Microsoft Edge by default, with optional Bing fallback search, for manual screenshot review; does not change analytical outputs. |
