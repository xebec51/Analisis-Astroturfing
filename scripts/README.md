...
| `open_tiktok_similarity_comments.py` | RM2 comment similarity review | LOCAL_BROWSER_REVIEW_AID | `comment_similarity_screenshot_queue.csv` | local lookup status/screenshots | Yes | No | No | No | Opens TikTok URLs in a local browser for manual screenshot review; does not change analytical outputs. |
