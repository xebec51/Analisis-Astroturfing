...
This registry separates canonical, frozen, legacy, and diagnostic entrypoints. Repository cleanup must not
rerun notebooks, model training, locked-test evaluation, or final inference.
...
| RM2 Sentiment V1 | Legacy sentiment/goals | `notebooks/legacy/02_rm2_sentiment_goals.ipynb` and `scripts/rm2_sentiment_goals_pipeline.py` | RM1 outputs | `output/rm2_sentiment/tables/` | LEGACY_V1 | No | Legacy outputs frozen | No | Kept for provenance and V1 versus V2 comparison. |
| Sentiment V2 annotation package | Human validation package | `scripts/build_rm2_sentiment_v2_annotation_package.py` | Dataset and legacy diagnostics | `output/rm2_sentiment/human_validation_v2/` | CANONICAL_RM2_SENTIMENT | No during cleanup | Human labels frozen | No | Do not overwrite human labels. |
...
| Sentiment V2 development training | Development model selection | `scripts/train_rm2_sentiment_v2_development_model.py` | Human development pool | `output/rm2_sentiment/model_v2/` | FROZEN_MODEL_TRAINING_DO_NOT_RERUN | No | Yes | No | Model already frozen. |
...
| Sentiment V2 final inference | Final full inference | `scripts/apply_rm2_sentiment_v2_final_inference.py` | Frozen model and dataset | `output/rm2_sentiment/final_v2/` | FINAL_INFERENCE_ALREADY_GENERATED | No during cleanup | Yes | No | Final V2 outputs already generated. |
...
- Do not rerun locked-test evaluation. It is recorded by `output/rm2_sentiment/model_v2/final_locked_test_evaluation_lock.json`.
- Do not retrain Sentiment V2 during repository cleanup.
- Do not run full inference during repository cleanup.
- Do not execute notebooks during repository cleanup.
