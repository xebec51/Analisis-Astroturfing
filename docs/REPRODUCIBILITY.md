...
- Package versions for final Sentiment V2 are stored in
  `output/rm2_sentiment/final_v2/FINAL_SENTIMENT_ANALYSIS_MANIFEST.json`.
...
| `scripts/verify_repository_integrity.py` | read-only audit | Safe to run. |
...
`output/rm2_sentiment/model_v2/final_locked_test_evaluation_lock.json`. Do not rerun locked-test evaluation,
...
refactors, documentation, and read-only verification are allowed. Any future rerun should be planned as a
separate analysis change with explicit hash and output-diff review.
